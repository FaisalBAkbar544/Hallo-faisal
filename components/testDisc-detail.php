...
// Ambil semua nilai most & least dari kolom yang tersedia
$most = [
    'D' => $data['most_d'] ?? 0,
    'I' => $data['most_i'] ?? 0,
    'S' => $data['most_s'] ?? 0,
...
// Ambil nilai change langsung dari kolom
$change = [
    'D' => $data['change_d'] ?? 0,
    'I' => $data['change_i'] ?? 0,
    'S' => $data['change_s'] ?? 0,
...
function drawCharts() {
    const drawLineChart = (elementId, title, dataPoints, color) => {
    ...
    drawLineChart('graph_most', 'MOST - Public Self', <?= json_encode($most) ?>, '#c0392b');
    drawLineChart('graph_least', 'LEAST - Private Self', <?= json_encode($least) ?>, '#e67e22');
    drawLineChart('graph_change', 'CHANGE - Perceived Self', <?= json_encode($change) ?>, '#2980b9');
}
</script>
